...
let matrix = Matrix4Utils.createXRotation(Math.PI);
matrix = Matrix4Utils.translate(matrix, [-50, -75, -15]);

for (let ii = 0; ii < geometryPoints.length; ii += 3) {
	var vector = Matrix4Utils.vectorMultiply(matrix, [
		geometryPoints[ii + 0],
		geometryPoints[ii + 1],
		geometryPoints[ii + 2],
		1,
	]);
	geometryPoints[ii + 0] = vector[0];
	geometryPoints[ii + 1] = vector[1];
	geometryPoints[ii + 2] = vector[2];
}
